...
void CalculateT_boxes(vector<vector<uint8_t>> &T_boxe)
{
	uint8_t word0,word1,word2;
...
void mixColumns(vector<uint32_t> &s,unsigned & numBlocks,vector<vector<uint8_t>> &T_boxes) {

	vector<uint32_t> a;

	uint8_t word0,word1,word2,word3;
	vector<uint32_t> tmp_vec(4);
	for (unsigned int k = 0 ; k < numBlocks ; k++)
	{
		for(unsigned int i = 0 ; i < 4 ; i++)
		{
				word0 = T_boxes[wbyte(s[4*k+(i%4)],0)][1] ^ T_boxes[wbyte(s[4*k+((i+1)%4)],1)][2] ^ T_boxes[wbyte(s[4*k+((i+2)%4)],2)][0] ^ T_boxes[wbyte(s[4*k+((i+3)%4)],3)][0];
				word1 = T_boxes[wbyte(s[4*k+(i%4)],0)][0] ^ T_boxes[wbyte(s[4*k+((i+1)%4)],1)][1] ^ T_boxes[wbyte(s[4*k+((i+2)%4)],2)][2] ^ T_boxes[wbyte(s[4*k+((i+3)%4)],3)][0];
				word2 = T_boxes[wbyte(s[4*k+(i%4)],0)][0] ^	T_boxes[wbyte(s[4*k+((i+1)%4)],1)][0] ^ T_boxes[wbyte(s[4*k+((i+2)%4)],2)][1] ^ T_boxes[wbyte(s[4*k+((i+3)%4)],3)][2];
				word3 = T_boxes[wbyte(s[4*k+(i%4)],0)][2] ^ T_boxes[wbyte(s[4*k+((i+1)%4)],1)][0] ^ T_boxes[wbyte(s[4*k+((i+2)%4)],2)][0] ^ T_boxes[wbyte(s[4*k+((i+3)%4)],3)][1];
				tmp_vec.at(i) = word(word0,word1,word2,word3);
		} 
		s[4*k+0] = tmp_vec.at(0);
		s[4*k+1] = tmp_vec.at(1);
		s[4*k+2] = tmp_vec.at(2);
		s[4*k+3] = tmp_vec.at(3);
	}
...
void aes(vector<uint32_t> in, vector<uint8_t> &out, vector<uint32_t> skey, unsigned numBlocks,vector<vector<uint8_t>> & T_boxes)
{
	addRoundKey(in, skey, 0,numBlocks);
	for (unsigned int i = 1 ; i < 10 ; i++)
	{
		mixColumns(in,numBlocks,T_boxes);
...
int main(int argc, char* argv[])
{
	...
	printf("Encryption using TBoxes \n\n");
	printf("Key: ");
	hexprint16(key,16);
	puts("");
	printf("In:  ");
	...
	CalculateT_boxes(T_boxes);
	expandKey(key, expKey);
	
	for (unsigned int i = 0 ; i < count ; i++)
	{
		aes(in, out, expKey, 4,T_boxes);
	}
